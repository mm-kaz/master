...
#rep(avr,early_2cell)の発現量を比較する

#ピアソン相関係数を算出する(+1はRPKM0だと駄目なため)
cor.test(log2(DATA$avr+1),log2(DATA$early_2cell+1),method="pearson")

# 「r={相関係数}」という文字列を作成する。ピアソン相関係数はroundする。
corr <- round(cor(log2(DATA$avr+1),log2(DATA$early_2cell+1),method="pearson"), 2)

#scattering plotを描く(ggplot2)
p <- ggplot(log2(DATA+1),aes(x=avr, y=early_2cell))+
  geom_point()+  #散布図
...
p <- p+
  xlab("average log2(RPKM+1)")+ #x軸ラベル
  ylab("reference log2(RPKM+1)")+ #y軸ラベル
  xlim(0,13)+ylim(0,13) #x軸とy軸の範囲指定

#scattering plotを出力する(解像度100dpi,幅6インチ,高さ6インチ)
ggsave(file="./Desktop//analysis/rna-seq/scatter2.png",plot=p,dpi=100,width=6,height=6)
